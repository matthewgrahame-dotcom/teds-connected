...
import { Circle, CircleDot, CheckCircle2, ExternalLink, ChevronDown, ChevronUp, ChevronLeft, PlayCircle, AlertTriangle, GraduationCap } from 'lucide-react';
...
export default function ProgramDetailPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const all: Program[] = await resp.json();
      setProgram(all.find((p) => String(p.id) === id) ?? null);
    } catch {
      setProgram(null);
    }
  };
  ...
  return (
    <div className="px-5 py-8 lg:px-10 lg:py-10">
      <div className="mx-auto max-w-3xl space-y-6">
        <Link href="/learn/programs" className="inline-flex items-center gap-1 text-sm font-semibold text-muted-foreground hover:text-foreground">
          <ChevronLeft className="h-4 w-4" /> Back to Training and Programs
        </Link>

        {program === undefined && <p className="text-sm text-muted-foreground">Loading…</p>}
        {program === null && <p className="text-sm text-muted-foreground">That program couldn't be found.</p>}

        {program && (
          <section className="overflow-hidden rounded-xl border border-card-border bg-card shell-shadow">
            <div className="aspect-[21/9] w-full shrink-0 overflow-hidden bg-muted">
